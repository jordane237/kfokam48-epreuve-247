...
| EF8 | Le système assigne automatiquement un relecteur à chaque exercice déposé | Quand un exercice est déposé, le système choisit au hasard un étudiant présent à la session, différent de l'auteur (RG13, RG14), et l'exercice passe à l'état `assigne` ; s'il n'y a aucun candidat, il reste `en_attente_relecteur` et apparaît comme tel dans le tableau (Q11, RG14) | Must |
| EF9 | Le relecteur soumet une note et un commentaire | Quand je poste `POST /api/relectures/{id}` avec une note entière 0–20 et un commentaire → `200` (RG8) ; note hors bornes ou non entière → `400 { code: "NOTE_INVALIDE" }` ; relecture déjà rendue → `409 { code: "RELECTURE_DEJA_RENDUE" }` (RG10) | Must |
...
| EF11 | L'étudiant relu voit sa note et le commentaire, sans le nom du relecteur | Quand je consulte mon exercice relu, j'affiche note et commentaire ; l'identité du relecteur n'est exposée par aucune API de consultation (Q8) | Must |
...
| EF13 | Le formateur consulte un tableau de suivi par promotion | Quand j'appelle `GET /api/tableau?promotionId=`, je reçois `200 [ { etudiantId, nom, presences, exercicesDeposes, moyenne, relecturesEnAttente } ]` ; moyenne calculée côté API, jamais recalculée côté front (F3) ; promotion inconnue → `404 { code: "PROMOTION_INCONNUE" }` (Q16, RG15) | Must |
...
| RG10 | Une relecture est définitive dès son envoi : plus aucune correction possible | Q15, tranché en faveur de Q15 (contradiction avec Q10, voir §7) |
...
| RG15 | Le tableau du formateur montre, par étudiant : sa présence à chaque session, le nombre d'exercices déposés, la moyenne des notes reçues, et les relectures qu'il doit encore faire | Q16 |
| RG16 | La relecture attendue est unique : un seul relecteur par exercice | Q6 |
